...
The CI workflow validates the site: every declared page exists, the logo is present, no credential files are committed, .gitignore covers private paths, and the patent notice is present. These checks run on every pull request and push to main.

Run the same checks locally:

```sh
Run the same checks locally with the `Site Validation` job steps, or simply verify the declared pages exist and no credential files are tracked.
```
